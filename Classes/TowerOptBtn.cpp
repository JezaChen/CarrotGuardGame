    ...
    for (auto& iterTower : LevelConfigUtil::getInstance()->getCurrentTowersInfo())
    {
        ...
        auto _Toweritem = MenuItemSprite::create(_TowerItemSp, _TowerItemSp, [=](Ref* r)
        {
            ...
            case 37:
                ...
                break;
            case 46:
                _towerType = en_MushroomTower;
                break;
            case 43:
                _towerType = en_FishTower;
                break;
            case 40:
        ...
        _pCreateTowerItemSpVec->pushBack(_Toweritem); //塞进向量
        //根据个数分行处理
...
void TowerOptBtn::gradeTowerMenu(TowerBase* touchTower)
{
    ...
    UpgradeItem->setTag(touchTower->getTowerGradeCost()); //标签标记的是成本
...
{
    //检查建塔菜单子项向量
    for (auto itemCreateSp = _pCreateTowerItemSpVec->begin();
